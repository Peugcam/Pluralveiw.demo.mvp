 ...
 *
 * Exemplos:
 * - "porque o bitcoin caiu hoje?" → detecta "hoje" → filtra por 2025-10-10
 * - "notícias de ontem sobre IA" → detecta "ontem" → filtra por 2025-10-09
...
export class TemporalDetector {
   ...
   * @returns {Object|null} - Informações temporais ou null se não encontrado
   */
  detect(query) {
   ...
   * @param {Object} temporalInfo - Informações temporais detectadas
   * @returns {boolean} - true se o resultado é válido
   */
  validateResult(result, temporalInfo) {
    ...
    try {
      ...
      if (!dateInContent) {
        ...
        return this.hasRecentKeywords(result.title + ' ' + result.content, temporalInfo)
      }
   ...
   */
  hasRecentKeywords(text, temporalInfo) {
